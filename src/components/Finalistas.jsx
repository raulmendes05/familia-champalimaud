...
import { isSupabaseConfigured, fetchWinnerVotes } from '../lib/supabase'

const MEDALS = ['🥇', '🥈', '🥉']

/**
 * Pódio "Reta Final": os últimos sobreviventes com dossier (linhagens + títulos)
 * e o placar de votos (só leitura — votar é em /votar).
 */
export default function Finalistas({ survivors, members, relationships }) {
  ...
  useEffect(() => {
    if (isSupabaseConfigured()) fetchWinnerVotes().then(({ tally }) => setTally(tally)).catch(() => {})
  }, [])
  const total = Object.values(tally).reduce((a, b) => a + b, 0)

  const ranked = [...dossiers].sort(
    (a, b) => (tally[b.m.id] || 0) - (tally[a.m.id] || 0) || a.m.name.localeCompare(b.m.name)
  )

  return (
    <section className="mb-8">
      <div className="mb-3 flex flex-wrap items-center justify-between gap-2 border-b border-champi-line pb-2">
        <h2 className="font-display text-xl font-semibold text-champi-text">
          🏆 Reta Final · {survivors.length} finalistas
        </h2>
        <Link to="/votar" className="btn-gold px-3 py-1.5 text-sm">
          🗳️ Apostar no vencedor
        </Link>
      </div>
      <p className="mb-4 text-xs text-champi-text-dim">
        {total > 0 ? `${total} voto${total === 1 ? '' : 's'} até agora.` : 'Ainda ninguém apostou.'} Vota em{' '}
        <Link to="/votar" className="text-champi-gold underline">
          /votar
        </Link>
        .
      </p>
          ...
          return (
            <li key={m.id} className="card p-4">
              <div className="flex items-center gap-3">
                <span className="w-6 shrink-0 text-center text-lg">{total > 0 ? MEDALS[i] || `${i + 1}º` : ''}</span>
                ...
                {total > 0 && <span className="shrink-0 text-sm font-bold text-champi-gold">{pct}%</span>}
              </div>
              ...
              {total > 0 && (
                <div className="mt-2.5 h-2 w-full overflow-hidden rounded-full bg-champi-ink-3">
